...
test_that("two-sided formula produces warning", {
  expect_warning(na_if_not(0:9, x ~ . < 1), "Formula arguments must be one-sided")
})
...
test_that("multiple non-coercible arguments produce multiple warnings", {
  expect_warning(na_if_not(0:9, mean, lm(1 ~ 1)), "`mean` unused")
  expect_warning(na_if_not(0:9, mean, lm(1 ~ 1)), "`lm(1 ~ 1)` unused", fixed = TRUE)
})
